...
// CSS filter strings — no longer used for rendering (kept for reference)
// Live preview now uses pre-rendered filtered canvases via getFilteredImage()
...
// Color matrices for pixel-accurate export (5x4 row-major, last col = offset)
...
function applyContrastToData(data, factor) {
...
function applySaturationToData(data, factor) {
  for (let i = 0; i < data.length; i += 4) {
    if (data[i + 3] === 0) continue; // skip transparent pixels
...
function applyFilterToImageData(imageData, filter) {
  ...
  if (filter === "grayscale") {
    ...
    applyContrastToData(d, 0.85);
  } else if (filter === "high-contrast") {
    applyContrastToData(d, 1.25);
  } else if (filter === "low-quality-display") {
    applyContrastToData(d, 0.95);
    applySaturationToData(d, 0.6);
...
// Web Worker pool for off-main-thread filter processing during export
const WORKER_POOL_SIZE = navigator.hardwareConcurrency || 4;
const filterWorkerPool = [];
let filterWorkerRoundRobin = 0;
let filterWorkerIdCounter = 0;
let filterWorkerBlobUrl = null;

function generateFilterWorkerSource() {
  return `
...
function getFilterWorkerBlobUrl() {
  if (filterWorkerBlobUrl) return filterWorkerBlobUrl;
  const blob = new Blob([generateFilterWorkerSource()], {
    type: "application/javascript",
  });
  filterWorkerBlobUrl = URL.createObjectURL(blob);
  return filterWorkerBlobUrl;
}

function initFilterWorkerPool() {
  if (filterWorkerPool.length > 0) return;
  const url = getFilterWorkerBlobUrl();
  for (let i = 0; i < WORKER_POOL_SIZE; i++) {
    ...
    filterWorkerPool.push(worker);
  }
}

function applyFilterViaWorker(imageBitmap, filter) {
  initFilterWorkerPool();
  const id = filterWorkerIdCounter++;
  const worker =
    filterWorkerPool[filterWorkerRoundRobin % filterWorkerPool.length];
  filterWorkerRoundRobin++;
  return new Promise((resolve, reject) => {
    worker._pending.set(id, { resolve, reject });
    worker.postMessage({ type: "apply", imageBitmap, filter, id }, [
      imageBitmap,
    ]);
  });
}

// Apply filter to the export canvas (pixel-accurate, for PNG export)
async function applyFilterToCanvas(canvasEl, filter) {
  if (!filter || filter === "none") return;
  const ctx2 = canvasEl.getContext("2d");
  const imageData = ctx2.getImageData(0, 0, canvasEl.width, canvasEl.height);
  applyFilterToImageData(imageData, filter);
